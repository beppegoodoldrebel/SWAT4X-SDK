...
var private vector InitialLocation;
...
latent function MoveToInitialLocation()
{
	if (CurrentMoveToLocationGoal != None)
	{
		CurrentMoveToLocationGoal.Release();
    ...
    CurrentMoveToLocationGoal = new class'MoveToLocationGoal'(movementResource(), achievingGoal.Priority, InitialLocation);
    assert(CurrentMoveToLocationGoal != None);
	CurrentMoveToLocationGoal.AddRef();
	...
	instantSucceed(); //we can finish here
}
...
private function StartInvestigateTimeoutTimer()
{
	...
	InvestigateTimeoutTimer.timerDelegate = MoveToInitialLocation;
...
Begin:
	...
	InitialLocation = m_Pawn.Location;
